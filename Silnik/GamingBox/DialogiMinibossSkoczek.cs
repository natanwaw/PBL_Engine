...
{
    class DialogiMinibossSkoczek : GameObjectComponent
    {
        GameObject miniboss;
        ...
        public override void Start()
        {
            ...
            miniboss.GetComponent<BossSkoczek>().isMovable = false;
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            if (gameObject.GetComponent<Dialog>()._dialogBox._currentPage == 6 && !czyPoWalce)
            {
                miniboss.GetComponent<BossSkoczek>().isMovable = true;
            }
            if (gameObject.GetComponent<Dialog>()._dialogBox._currentPage == 6 && czyPoWalce)
            {
                gameObject.GetComponent<Dialog>()._dialogBox.Show();
            ...
            if (gameObject.GetComponent<Dialog>()._dialogBox._currentPage == 14)
            {
                gameObject.GetComponent<Dialog>()._dialogBox.Show();
            }
            if (gameObject.GetComponent<Dialog>()._dialogBox._currentPage == 15)
            {
                gameObject.GetComponent<Dialog>()._dialogBox.Show();
            }
            if (gameObject.GetComponent<Dialog>()._dialogBox._currentPage == 16)
            {
                gameObject.GetComponent<Dialog>()._dialogBox.Show();
            }
            if (gameObject.GetComponent<Dialog>()._dialogBox._currentPage == 17)
            {
                gameObject.GetComponent<Text>()._textBox.Hide();
                GameObject.Destroy(miniboss);
            }

            if ((gameObject.GetComponent<Dialog>()._dialogBox._currentPage == 17 && distance(bramaSkoczek.GetComponent<Transform>().position, gameObject.GetComponent<Transform>().position) < 10) || cameraExit)
            {
                gameObject.GetComponent<Hero>().isMovable = false;
                cameraExit = true;
                if (firstEnter)
